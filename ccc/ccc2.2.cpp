...
vector<string> schedules; // schedules[i] = schedule of caretaker i
                          // '.' = available, 'X' = busy
int max_possible_sleep = 0; // Maximum feasible sleep length (integer) across all caretakers
...
/**
 * Compute the maximum possible sleep length (integer) that could ever be feasible.
 * Each caretaker must have a continuous '.' segment of at least length T.
 * Therefore T cannot exceed the minimum over caretakers of their longest free segment.
 */
...
set<Fraction> GenerateCandidateFractions(int maxSleepTime)
{
    ...
    for (int q = 1; q <= 20; q++)
    {
        ...
        {
            Fraction f(p, q);
            // Only keep fractions <= maxSleepTime
            if (f <= maxSleepTime)
            {
                fractions.insert(f);
